...
module Narra
  module API
    module Modules
      class ProjectsV1Copy < Narra::API::Module
        ...
        resource :projects do
          ...
          post ':identifier/copy' do
            ...
            return_one_custom(Project, :identifier, false, [:author]) do |project, roles, public|
              ...
              destination = Narra::Project.find_by(identifier: params[:destination])
              # check existence
              error_not_found! if destination.nil?
              # copy
              event = Narra::Core.copy(project, destination, current_user._id.to_s)
              # present
